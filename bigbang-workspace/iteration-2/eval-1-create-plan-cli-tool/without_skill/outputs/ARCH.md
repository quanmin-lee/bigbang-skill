...
本项目已有：
- `src/` — 核心代码
- `skills/` — 技能模块（含 bigbang 自身）
- `configs/` — 配置文件
- `tests/` — 测试目录
- `data/` — 数据目录
- `scripts/` — 脚本

新 CLI 工具将作为独立模块 `tools/excel-summarizer/` 放置，不侵入现有业务代码。
...
| 测试 | `tools/excel-summarizer/tests/` | 低 | 单元测试 + 集成测试 |
| setup | `pyproject.toml` 或 `setup.py` | 低 | 可选，用于 `pip install -e .` 安装为 CLI 命令 |
...
- **空列处理**：如果指定列没有数值数据或列不存在，需给出清晰的错误信息
- **CSV 编码**：输出 CSV 默认用 UTF-8，确保中文 Excel 内容不乱码
- **文件路径不存在**：`--input` 路径不存在时给出友好的错误提示
