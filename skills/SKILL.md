...
4. **验收检查 — Check-it**（❗ 新增）
   当本批次触发了 EXECUTION_PLAN.md 中定义的检查点时，必须执行验收检查。
   ...
   **执行检查**：
   ...
   4. Checker 执行验证，产出 `CHECK_REPORT.md`

   **结果判定**（读 CHECK_REPORT.md 的综合判定）:

   | 判定 | 处理方式 |
   |------|---------|
   | ✅ 通过 | 继续下一步 |
   | ⚠️ 有偏差但不阻塞 | 记录已知问题到已知问题清单，继续下一步 |
   | ❌ 不通过 | **暂停**执行 → 通知 PM → PM 决定：A. 微调任务后可继续 / B. 重新规划 / C. 记录为已知问题 |

   **重要**：check-it 是"验收"不是"验证"。它检查"功能是否满足 PRD 需求"（用户视角），不是"代码是否正确"（那是测试的事）。
